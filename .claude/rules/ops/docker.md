...
- **Install.** `RUN --mount=type=cache,target=/root/.cache/uv uv sync --frozen --no-dev`.
  Use `--frozen` (installs `uv.lock` exactly, no re-resolve; matches CI) - not `--locked`,
  because the relative `exclude-newer` window makes `--locked` re-resolution drift over time.
  Locking happens only in `uv lock` / `make lock`.
