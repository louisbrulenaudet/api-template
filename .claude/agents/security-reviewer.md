...
description: Use PROACTIVELY before a release, when touching auth / settings / secrets / CORS / Docker / the agent harness, or when adding any path that handles client or personal data. Reviews for credential leakage, fail-open configuration, error-envelope disclosure, and legal-sector data-handling violations. Read-only: no Bash, no Edit. The caller passes a diff path outside the repo, or a list of files to audit.
