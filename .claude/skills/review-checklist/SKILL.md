...
The rule files under `.claude/rules/` are the authority. This is the **traversal order** for reviewing a diff against them, plus the diff-level signals that indicate a violation. It does not restate the rules - open the cited file when a call is close.

Why this exists as a skill rather than prose in each agent: only `core/guardrails.md` is always-on; the other 11 rule files are path-scoped via `paths:` frontmatter and may not load for a reviewer that only reads. Preloading this via a subagent's `skills:` field guarantees the criteria arrive.
...
Semantic bug-hunting (logic errors, race conditions, edge cases) belongs to the bundled `/code-review` skill. End-to-end behaviour belongs to `/verify`. Do not duplicate either - a reviewer prompted to find problems will invent them, and over-reporting trains the reader to ignore the report.
