---
description: Architecture review - layering, dependency direction, import cycles, module coupling/cohesion, Docker/Makefile build parity. Outputs a plan only.
...
This file is a Claude Code slash command: plain Markdown in `.claude/commands/`. When the user runs `/review-architecture` in chat, this content is sent as the prompt and runs in the current conversation.

- **Parameters:** `$ARGUMENTS` is additional scope-e.g. `/review-architecture endpoints wiring`, `/review-architecture error handling`, `/review-architecture Docker/compose parity`-narrow the review accordingly. If empty, assume full architecture review for this repo.

This command is project-scoped and works alongside the path rules in `.claude/rules/`. For broader reviews use `/review` instead.
...
1. **Gather scope** - Full architecture review, or narrow scope based on `$ARGUMENTS`.
...
- [ ] Scope clear (full repo or `$ARGUMENTS`-narrowed)
...
- Read: `app/main.py`, `app/api/v1/router.py`, `app/api/v1/endpoints/*`, `app/core/*`, `app/dtos/*`, `app/enums/*`, `app/exceptions/*`, `app/utils/*`, `Dockerfile`, `compose.yaml`, `Makefile`, `make/*.mk`, `AGENTS.md`.
- Grep for: dependency/import snippets, global middleware/exception handler mappings, or export statements.
- Consult `git diff` / `git log` when reviewing recent architectural changes for regressions.
- Consult the path rules in `.claude/rules/` (notably `backend/fastapi-routes.md`, `backend/exceptions.md`, `backend/middleware.md`, `ops/docker.md`) - only `core/guardrails.md` is always-on, the others are path-scoped and worth opening explicitly.

This review is read-only: do not run mutating commands. `make format` and `make ci` rewrite files (`ruff format .` plus `ruff check . --fix`); the non-mutating equivalents are `make check` and `make type-check`.

If context is insufficient, say which files you still need to read.
