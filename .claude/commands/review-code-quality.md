---
description: Code-quality review of app/ + tests/ - naming, strict type hints, Ruff compliance, Pydantic v2 usage, CoreError patterns, async correctness. Outputs a plan only.
...
This file is a Claude Code slash command: plain Markdown in `.claude/commands/`. When the user runs `/review-code-quality` in chat, this content is sent as the prompt and runs in the current conversation.

- **Parameters:** `$ARGUMENTS` is scope-e.g. `/review-code-quality naming`, `/review-code-quality types`, `/review-code-quality errors`, `/review-code-quality async`, `/review-code-quality tests`-narrow accordingly. If empty, assume full code quality review for this repo (`app/` + `tests/`).
...
Align with root `AGENTS.md` as the source of truth for conventions and patterns. The path rules in `.claude/rules/quality/python-style.md`, `.claude/rules/quality/python-tooling.md`, `.claude/rules/quality/testing.md`, and `.claude/rules/contracts/pydantic-dtos.md` carry the detail - open them explicitly, since only `.claude/rules/core/guardrails.md` is always-on and the rest are path-scoped. Deeper Pydantic guidance lives in the `.claude/skills/` skills.
...
1. **Gather scope** - Full code quality review or the narrower scope given in `$ARGUMENTS`.
...
This review is read-only: read and reason about the code rather than changing it, and do not run mutating commands. `make format` and `make ci` rewrite files (`ruff format .` plus `ruff check . --fix`); the non-mutating equivalents are `make check` and `make type-check`.

## Checklist

- [ ] Scope clear (full `app/` + `tests/` or the sub-scope in `$ARGUMENTS`)
