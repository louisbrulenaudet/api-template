...
This file is a Claude Code slash command: plain Markdown in `.claude/commands/`. When the user runs `/review-configuration` in chat, this content is sent as the prompt and runs in the current conversation.

- **Parameters:** `$ARGUMENTS` is scope-e.g. `/review-configuration env only`, `/review-configuration docker`, `/review-configuration ruff/pydantic`, `/review-configuration ports/healthcheck`-narrow accordingly. If empty, assume all configuration (env/secrets, Docker/Compose, uv/Ruff/Pydantic, build modes, parity).
...
Align with root `AGENTS.md` for environment and port allocation guidance. The path rules in `.claude/rules/backend/settings-config.md`, `.claude/rules/ops/docker.md`, `.claude/rules/ops/ci.md`, and `.claude/rules/quality/python-tooling.md` carry the detail - open them explicitly, since only `.claude/rules/core/guardrails.md` is always-on and the rest are path-scoped.
...
Do not open or print the contents of `.env` or any other secret-bearing file; review its presence, gitignore/dockerignore coverage, and the variable names the code expects instead.
...
1. **Gather scope** - Full configuration review or the narrower scope given in `$ARGUMENTS`.
...
- Read the config files: `compose.yaml`, `Dockerfile`, `.env.template`, `.gitignore`, `.dockerignore`, `pyproject.toml`, `uv.lock`, `app/core/config.py`, `app/main.py`.
- Grep for env/parsing snippets:
  - `Settings` initialization + env validation in `app/core/config.py`
  - `load_dotenv()` behavior in `app/main.py`
- Consult `git diff` / `git log` when checking recent configuration changes for quality regressions.

This review is read-only: do not run mutating commands and do not edit config. `make format` and `make ci` rewrite files (`ruff format .` plus `ruff check . --fix`); the non-mutating equivalents are `make check` and `make type-check`. Never hand-edit `uv.lock` - regenerate it with `make lock` if the review concludes it is stale.

If context is insufficient, say which config files you still need to read.
