...
This project avoids the FastAPI Cloud CLI stack (`fastapi-cloud-cli` / `sentry-sdk`) by depending on `fastapi-cli[standard-no-fastapi-cloud-cli]` rather than `fastapi[standard]` — and that dependency lives in its own `devserver` dependency group, not `[project] dependencies`, so the CLI and its typer/click/rich tail never reach the production image at all. The runtime dependency list is likewise narrower than `fastapi[standard]` on purpose: no `jinja2` (and so no `markupsafe`), because nothing renders a template and Starlette imports Jinja lazily. `python-multipart` **does** stay, even though no route declares a `Form()` or `UploadFile` — `starlette.requests` imports `starlette.formparsers` at module scope, which imports it unconditionally, so `import fastapi` fails without it. Neither `fastapi` nor `starlette` declares that dependency, which is why it is pinned explicitly here. The `runtime` stage runs `uvicorn` directly; only `make dev` and the Docker `reload` stage use the CLI. `devserver` is separate from `dev` so the `reload` stage can take just the CLI (`uv sync --no-dev --group devserver`) without `uv`, pre-commit, pytest, Ruff and ty — `dev` pulls it back in via `include-group`, so `make dev` and `make sync` need no extra flag. Logfire export is disabled by default via environment variables in the Makefile, Docker, Compose, and CI. **uv** does not document a switch to turn off PyPI User-Agent metadata; see [astral-sh/uv#8474](https://github.com/astral-sh/uv/issues/8474).
